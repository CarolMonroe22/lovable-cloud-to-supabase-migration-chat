...
description: Migrate a Lovable Cloud project to a standalone Supabase project from claude.ai chat (the consumer Claude product, not Claude Code). Automates schema, data, auth, storage, and edge functions migration via Lovable MCP and Supabase MCP. Frontend code migration is delegated to Claude Code on the web or local terminal because GitHub access is not available in claude.ai chat. Use this skill when the user asks to migrate, export, or move a Lovable Cloud project to their own Supabase, especially when they mention being in claude.ai chat or not having Claude Code installed. Keywords - migrate Lovable Cloud, export from Lovable, leave Lovable Cloud, Lovable to Supabase, migrate Lovable database.
license: MIT
---
...
| 8. Frontend code | **Delegated** — see "Frontend migration" section below | Manual user step |
| 9. Verify | Run a comprehensive audit query | Fully automatic |
...
- Session 2: Phases 7, 9 (edge functions + verification)

## Frontend migration: why it's delegated

The frontend codebase (typically 200+ files) cannot be faithfully reproduced from claude.ai chat:
- No GitHub MCP exists for the consumer Claude product
- `Lovable MCP:remix_project` clones the project but inherits Lovable Cloud (won't connect to your new Supabase)
...
**Two recommended options for the user:**

**Option 1: Claude Code on the web (recommended, fastest)**
Direct the user to [claude.ai/code](https://claude.ai/code), where they can:
1. Connect their GitHub account (one-time setup)
2. Run the original `lovable-cloud-to-supabase-migration` skill
...
In both options, after the code lands in the new Lovable repo, send a follow-up `Lovable MCP:send_message` to trigger a rebuild against the new Supabase.
...
3. `Lovable MCP:read_file` for `package.json` at the latest SHA — **detect tech stack**:
   - If `name === "vite_react_shadcn_ts"` → use `tech_stack: "classic"` in Phase 8
...
### Phase 8: Frontend code (delegated)

This skill **does not migrate frontend code from claude.ai chat**. See "Frontend migration" section at the top of this file for the rationale.

Instead, generate this guidance for the user:

> **Phase 8 — Frontend migration (manual)**
> 
...
> 3. Run the `lovable-cloud-to-supabase-migration` skill there
...
**This skill CAN automate Steps 30 and 34 of the Code skill** (creating the new empty Lovable project and triggering rebuild). Offer them to the user:

1. `Lovable MCP:create_project` (Step 30):
...
When all 9 phases pass verification:
1. Tell the user the new Supabase project is ready
2. Remind them to set edge function secrets in the dashboard (from Phase 7's list)
3. Tell the user to disconnect the Lovable project from Lovable Cloud and reconnect it to the new Supabase project via the Lovable dashboard (Settings → Database). Lovable Cloud cannot be "disabled" — once activated, it remains attached. The user switches the Supabase connection to point at their new project instead.
4. Optional: offer to delete the temporary `migrate-storage` edge function from Phase 6
