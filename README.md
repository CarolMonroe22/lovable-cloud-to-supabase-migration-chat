...
[![Version](https://img.shields.io/badge/version-2.0.0-FF5CD7)](https://github.com/CarolMonroe22/lovable-cloud-to-supabase-migration-chat)
...
> **Update (July 2026):** Lovable Cloud now has official **Export**, **Pause** and **Remove** buttons, and v2 of this skill is built around them. The endgame is now the same-project switch: migrate the backend via MCP, verify, then Remove Cloud and connect your own Supabase to the SAME Lovable project - no code push. If you have Claude Code, the [main migration skill v4](https://github.com/CarolMonroe22/lovable-cloud-to-supabase-migration) is still the most complete route; the [current walkthrough](https://carolmonroe.com/blog/export-remove-lovable-cloud) has every dashboard step.
...
| **What doesn't** | Frontend code - it doesn't need to. You keep the SAME Lovable project and just switch its backend |
...
| You don't have `gh` or `git` installed | Yes (the default flow needs no code push at all) |
...
| 8. Verify | Compares 12 categories between source and destination, scans for old URL leaks - BEFORE anything is removed |
| 9. The Switch | You download the official Export backup, Remove Lovable Cloud, and connect your own Supabase to the same project (dashboard clicks, guided) |
...
| Frontend code | Stays put - the same project keeps its code (fallback path delegates a code push) |
...
| Phase 8 verification | Migration appears complete but pieces are missing |
...
- The official Export `.backup` file can't be restored from claude.ai chat (needs `pg_restore` locally) - here it's your safety backup, the data moves via MCP
- The fallback path (keeping the Cloud project untouched) requires Claude Code or your local terminal for the code push
...
| Current walkthrough (Export + Remove + Connect) | [carolmonroe.com/blog/export-remove-lovable-cloud](https://carolmonroe.com/blog/export-remove-lovable-cloud) |
| Migrate with AI guide (chat + Code routes) | [carolmonroe.com/blog/migrate-lovable-cloud-with-ai](https://carolmonroe.com/blog/migrate-lovable-cloud-with-ai) |
